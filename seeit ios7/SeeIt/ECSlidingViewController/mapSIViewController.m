...
//        CLCircularRegion *region = [[CLCircularRegion alloc] initWithCenter:location radius:20 identifier:[rows[i] objectForKey:@"placeName"]];
...
//           }
    ...
    {
        CLLocationCoordinate2D location;
        location.latitude = [[entry objectForKey:@"latitude"] floatValue];
        location.longitude = [[entry objectForKey:@"longitude"] floatValue];
        
        //create and add regions
        CLCircularRegion *region = [[CLCircularRegion alloc] initWithCenter:location radius:20 identifier:[entry objectForKey:@"placeName"]];
        [regionLocations addObject:region];
    }
...
-(MKAnnotationView *)mapView:(MKMapView *)mapView viewForAnnotation:(id<MKAnnotation>)annotation
{
    ...
    [self.info addTarget: self
                       action: calloutAccessoryTappedSelector
...
- (void)locationManager:(CLLocationManager *)manager didUpdateToLocation:(CLLocation *)newLocation fromLocation:(CLLocation *)oldLocation
{
    if (currentLocation == nil)
    {
        ...
        NSSet *filterlocations = [NSSet setWithArray:rows];
        
        NSSet *nearbyLocations = [filterlocations objectsPassingTest:^(id obj, BOOL *stop)
                                  {
                                      CLLocation *location = [[CLLocation alloc] initWithLatitude:[[obj objectForKey:@"latitude"] floatValue] longitude:[[obj objectForKey:@"longitude"] floatValue]];
                                      BOOL result = [newLocation distanceFromLocation:location] <= 10000;
                                      return result;
                                  }];
        
        for (NSDictionary *entry in nearbyLocations)
        {
            CLLocationCoordinate2D location;
            SIAnnotation * myAnn = [[SIAnnotation alloc]init];
            location.latitude = [[entry objectForKey:@"latitude"] floatValue];
            location.longitude = [[entry objectForKey:@"longitude"] floatValue];
            myAnn.coordinate = location;
            myAnn.title = [entry objectForKey:@"placeName"];
            myAnn.subtitle = @"Info";
            [locations addObject:myAnn];
        }
